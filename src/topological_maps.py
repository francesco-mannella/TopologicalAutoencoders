...
class TopologicalMap(torch.nn.Module):
    ...
    def __init__(self, input_size, output_size, output_dims=2, parameters = None):
        ...
        self.curr_std = torch.nn.Parameter(torch.zeros(1).float(), requires_grad=False)
    ...
    def forward(self, x: torch.Tensor, std: float = None) -> torch.Tensor:
        ...
        self.phi = self.radial(self.bmu, std)
        self.norms = norms

        return norms2*self.phi
